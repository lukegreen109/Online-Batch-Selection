...
class SelectionMethod(object):
    ...
    def before_batch(self, i, inputs, targets, indexes, epoch) -> BatchInfo:
    ...
    def after_batch(self, i, inputs, targets, indexes, outputs):
        ...
        self.num_selected_noisy_indexes = (
            self.num_selected_noisy_indexes
            + np.intersect1d(indexes, self.noisy_indices).size
        )
    ...
    def train(self, epoch, list_of_train_idx):
        ...
        for i, datas in enumerate(self.train_loader):
            ...
            batch = self.before_batch(
                i, batch_inputs, batch_targets, batch_indexes, epoch
            )
            ...
            selected_outputs, features = (
                ...
                if self.need_features
                    ...
                    self.model(
                        x=batch.inputs, need_features=False, targets=batch.targets
                    ),
                    None,
                )
            )
            ...
            if batch.weights is None:
                # Unweighted loss, perform mean reduction
                loss = self.criterion(selected_outputs, batch.targets, reduction="mean")
                loss = torch.mean(loss)
            else:
                # Weighted loss (weighted sum of per-sample losses)
                loss_by_sample = self.criterion(
                    selected_outputs, batch.targets, reduction=None
                )
                loss_by_sample *= batch.weights
                loss = torch.sum(loss_by_sample)
            ...
            self.after_batch(
                i, batch.inputs, batch.targets, batch.indices, selected_outputs.detach()
            )
    ...
    def log_statistics(self, epoch):
        ...
        self.logger.info(
            f"=====> Epoch: {epoch}/{self.epochs}, Total_step: {self.total_step}, lr: {self.optimizer.param_groups[0]['lr']:.6f}, Total Time: {self.total_time:.4f} s, Time this epoch: {self.time_this_epoch:.4f} s"
        )
        self.logger.info(
            f"=====> Epoch: {epoch}/{self.epochs}, Train Loss: {train_loss:.4f}, Train acc: {train_acc:.4f}, Best val acc: {self.best_acc:.4f}, Current val acc: {val_acc:.4f}, EMA val acc: {ema_val_acc}"
        )
        ...
        self.logger.wandb_log(
            {
                ...
                "time_epoch": self.time_this_epoch,
                "percent noisy points selected": self.num_selected_noisy_indexes
                / len(self.train_dset),
            }
        )
    ...
    def test_val(self):
        ...
        model = self.model.module if hasattr(self.model, "module") else self.model
        ema_model = (
            self.ema_net.ema_module
            if hasattr(self.ema_net, "ema_module")
            else self.ema_net
        )
        model.eval()
        all_preds = []
