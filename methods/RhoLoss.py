...
from methods.SelectionMethod import BatchInfo
...
class RhoLoss(SelectionMethod):
    ...
    def __init__(self, config, logger):
        ...
        self.uniform_epochs = (
            config["method_opt"]["uniform_epochs"]
            if "uniform_epochs" in config["method_opt"]
            else 0
        )
    ...
    def setup_holdout_model(self, config, logger):
        ...
        if teacher_model_source == "clip":
            ...
            try:
                self.teacher_model = getattr(models, teacher_model_type)(
                    **teacher_model_args
                )
    ...
    def reducible_loss_selection(
        self, inputs, targets, indexes, selected_num_samples, epoch
    ):
        ...
        irreducible_loss = self.train_dset.irreducible_loss_cache[indexes].to(
            total_loss.device
        )
        ...
        _, indices = torch.topk(
            reducible_loss, selected_num_samples, largest=True, sorted=False
        )
    ...
    def before_batch(self, i, inputs, targets, indexes, epoch):
        ...
        indices = self.reducible_loss_selection(
            inputs, targets, indexes, selected_num_samples, epoch
        )
