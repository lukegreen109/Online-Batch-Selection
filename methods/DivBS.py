...
class DivBS(SelectionMethod):
    ...
    def __init__(self, config, logger):
        ...
        self.reduce_dim = config['method_opt'].get('reduce_dim', False)
        
    def get_ratio_per_epoch(self):
        epoch = self.epoch  # use class attribute
    ...
    def before_batch(self, i, inputs, targets, indexes):
        ...
        if ratio == 1.0:
            ...
            if i == 0:
                ...
                self.logger.info(f'selecting samples for epoch {self.epoch}, ratio {ratio}')
