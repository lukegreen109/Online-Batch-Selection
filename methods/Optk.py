...
class Optk(SelectionMethod):
    ...
    def precompute_losses(self):
        ...
        self.logger.info(
            f"Cached irreducible losses for {len(losses_tensor)} samples in dataset."
        )
        ...
        if teacher_model_path == "None":
            ...
            try:
                ...
                self.teacher_model.load_state_dict(
                    torch.load(teacher_model_path, map_location=self.device)
                )
    ...
    def train_teacher_model(self, config, logger):
        ...
        logger.info(
            f"[teacher Model] Starting training for {epochs} epochs on {total_batch} batches per epoch."
        )

        best_loss = float("inf")
        best_model_path = os.path.join(self.config["save_dir"], "best_teacher.pth.tar")
        ...
        for epoch in range(epochs):
            ...
            for i, datas in enumerate(self.teacher_dataloader_augmented):
                ...
                if (i + 1) % 50 == 0 or (i + 1) == total_batch:
                    logger.info(
                        f"[teacher Model][Epoch {epoch + 1}/{epochs}] "
            ...
            logger.info(
                f"[teacher Model][Epoch {epoch + 1}/{epochs}] Validation Loss: {avg_val_loss:.4f}"
            )
    ...
    def optk_loss_selection(self, inputs, targets, indexes, selected_num_samples):
        ...
        irreducible_loss = self.original_train_dset.irreducible_loss_cache[indexes].to(
            total_loss.device
        )
        ...
        _, indices = torch.topk(
            optk_loss, selected_num_samples, largest=True, sorted=False
        )
    ...
    def before_batch(self, i, inputs, targets, indexes, epoch):
        ...
        indices = self.optk_loss_selection(
            inputs, targets, indexes, selected_num_samples
        )
